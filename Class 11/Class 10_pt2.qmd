...
The `msa` package is from the BioConductor. These packages focus on genomics type of work and are managed by the `BiocManager` package 
...
Side note: Let's annotate these structures (in other words, find out what they are, what species they're from. stuff about the experiement they were solved in, etc.)

For this use the `pdb.annotate()`
```{r}
